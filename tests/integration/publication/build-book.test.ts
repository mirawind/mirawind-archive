...
import {
  ...
  type BuildRegistrationCrashPoint,
} from "@/modules/publishing/adapters/sqlite/build-registration";
...
import type { BuildTreeCrashPoint } from "@/modules/publishing/application/build-durability";
import {
  buildIdentities,
  finalizeBuild,
...
function commandFor(suffix: string): BuildBookCommand {
  const jobId = `job_candidate_builder_${suffix}`;
  return parseBuildBookCommand({
    reuse: null,
    bookId,
...
async function registrationFixture(
  ...
  const command = await writeCandidateInput(
    dataRoot,
    fixture.candidates.buildCommand(fixture.build.id),
  );
...
describe("isolated candidate child builder", () => {
  it("builds one immutable preview/public candidate with bounded telemetry", async () => {
    ...
    try {
      ...
      const [manifest, marker] = await Promise.all([
        readFile(
          resolve(buildDirectory, "document-manifest.json"),
          "utf8",
        ).then((value) => validateDocumentManifest(JSON.parse(value))),
        readFile(resolve(buildDirectory, "version.json"), "utf8").then(
      ...
      expect((await stat(buildDirectory)).mode & 0o777).toBe(0o500);
      expect(
        (await stat(resolve(buildDirectory, "preview/pages/1.html"))).mode &
          0o777,
      ).toBe(0o400);
      await expect(
  ...
  it("removes staging and never exposes a ready tree after cancellation", async () => {
    ...
    try {
      ...
      expect(
        await readdir(
          resolve(dataRoot.layout.bookDirectory, String(bookId), "builds"),
        ).catch(() => []),
      ).toEqual([]);
    } finally {
      await dataRoot.cleanup();
        ...
        await expect(
          handleBuildBook({
            ...
            execute: ({ command: parsed, onStage, signal }) =>
              buildBookVersion({
                command: parsed,
                crashPoint(crashPoint: BuildTreeCrashPoint) {
  ...
  it.each(["after_version_before_job"] as const)(
    ...
    async (point) => {
      ...
      try {
        ...
        await expect(
          finalizeBuild({
            ...
            registration: new BuildRegistrationRepository(
              ...
              (crashPoint: BuildRegistrationCrashPoint) => {
                if (crashPoint === point) throw new Error(`CRASH_${point}`);
              },
            ),
          }),
        ...
        expect(fixture.candidates.require(command.versionId)).toMatchObject({
          state: "building",
        });
  ...
  it("returns an already committed ready registration after its response is lost", async () => {
    ...
    try {
      ...
      expect(fixture.candidates.require(command.versionId).state).toBe("ready");
  ...
  it("rejects a completed tree after a newer candidate becomes current", async () => {
    ...
    try {
      ...
      migrated.database
        .prepare(
          "UPDATE book_documents SET updated_at=updated_at+1 WHERE book_id=?",
        )
        .run(command.bookId);
      const newer = fixture.candidates.createForDocument({
        bookId: command.bookId,
        sourceUpdatedAt: command.sourceUpdatedAt + 1,
        nowMs: createdAtMs + 1,
      ...
      expect(fixture.candidates.require(command.versionId).state).toBe(
        "building",
      );
      expect(fixture.candidates.findCurrent(command.bookId)?.id).toBe(newer.id);
    } finally {
      migrated.close();
      await dataRoot.cleanup();
    }
